    ...
    while(True):
        ...
        try:
            ...
            if H > 10 and H < 34:
                color = "Jaune"
                print('Jaune')
                # If the color is yellow, then don't produce the pulse
                ...
                color = "Vert"
                print('Vert')
                # If the color is green, then produce the pulse at pin16
                GPIO.setmode(GPIO.BOARD)
                GPIO.setwarnings(False)
                ...
                GPIO.cleanup()
            elif H > 112 and H < 140:
                color = "Bleu"
                print('Bleu')
                # If the color is blue, then produce the pulse at pin18
                GPIO.setmode(GPIO.BOARD)
                GPIO.setwarnings(False)
                ...
                color = "Rouge"
                print('Rouge')
                # If the color is red, then produce the pulse at pin15
                GPIO.setmode(GPIO.BOARD)
                GPIO.setwarnings(False)
                GPIO.setup(15, GPIO.OUT)
            ...
            cv2.putText(frame, text, (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (128, 128, 128), 2)
        except: pass
        

        # Display the frame with the color and shape
        cv2.rectangle(frame, (rect_x, rect_y), (rect_x+rect_w, rect_y+rect_h), (0, 255, 0), 2)
        cv2.drawContours(frame, contours, -1, (128, 128, 128), 3)
